...
/**
 * A class that defines the network parameters of the host. Actually, for the
 * purpose of this pedagogical exercise, we make some pretty significant
 * assumptions:
 *
 * First we consider that the interface that is directly connected to the
 * default gateway is our 'main/primary' interface. Second, we assume that there
 * is only one.
 *
 * Of course, neither assumption is 100% accurate but it is a good enough
 * assumption in the context of our work. It allows us to concentrate on a
 * single NIC when we do things such as passively listen to tcp connections and
 * when we run a scan of our network.
 *
 * @author fbegin1
 *
 */
 ...
 * We get the network address for the main interface.
    ...
    public void findDNSdata(InetAddress myPublicIP) {
        ...
        // Find the domain (assumption here is .com, .net, etc so we use the last 2 parts of the FQDN)
        String[] nameParts = hostFQDN.split("\\.");
        hostDomainName = nameParts[nameParts.length-2] + "." + nameParts[nameParts.length - 1];
    ...
    /**
     * A method that takes a string containing the binary representation of an
     * IP address and changes it to the usual decimal representation
     *
     * @param myBinaryIP
     * @return
     */
    ...
    /**
     * Method that dos a logical AND on two strings representing binary numbers
     *
     * @param firstString
     * @param secondString
     * @return
    ...
    /**
     * Method that translates a byte array (representing an IP) into a binary
